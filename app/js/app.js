...
var initRecognizer2 = function() {
	// You can pass parameters to the recognizer, such as : {command: 'initialize', data: [['-hmm', 'my_model'], ['-fwdflat', 'no']]}
	postRecognizerJob({command: 'initialize', data: [['-kws_threshold', '1e-35'], ['-kws', 'keyphrase.list'], ['-dict', 'keyphrase.dict']]},
			function() {
				if (recorder) recorder.consumers = [recognizer];
				// feedWords(wordList);
			}
	);
	postRecognizerJob({command: 'load', callbackId: 0, data: ['keyphrase-list.js']})
};

function initRecognizer3() {
	recognizer.postMessage({
		command: 'lazyLoad',
		...
		data: {
			folders: [['/', 'modelFiles']],
			files: [['/modelFiles', 'keyphrase.list', '/../lib/keyphrase.list'],
					['/modelFiles', 'keyphrase.dict', '/../lib/keyphrase.dict']]
		}
	});
}
...
var initRecognizer = function() {
	postRecognizerJob({
		command: 'load',
		data: ['load-data.js'] //this path is relative to the web-worker!
	},
	function() {
		postRecognizerJob({
			command: 'initialize',
			data:[ //these paths are the emscripten virtual paths you setup in load-data.js
				['-kws_threshold', '1e-35'],
				['-kws', 'keyphrase.list'],
				['-dict', 'keyphrase.dict']
			]
...
// var wordList = [['MAGIC', 'M AE JH IH K'], ['WHETHER', 'W EH DH ER'], ['WEATHER', 'W EH DH ER'], ['CLOSE', 'K L OW ZH']];
// var keywords = [{title: 'Open Weather', g: 'MAGIC WHETHER'}, {title: 'Close All', g: 'MAGIC CLOSE'}, {title: 'Open Weather', g: 'MAGIC WEATHER'}];
var keywordIds = [];
